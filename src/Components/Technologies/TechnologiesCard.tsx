...
const TechnologiesCard = ({ tech, added, setAdded }: tTechCardMain) => {
 ...
 const isAlreadyAdded=added.find((item)=>item.name===tech.name)
  const handleIsStacked = () => {
    
    if(isAlreadyAdded){
      toast.error('Duplicated Detected')
      return
    }
    setAdded([...added, tech]);
    ...
    <div>
      <div className="max-w-sm rounded-2xl bg-base-100 p-6 shadow-xl border border-base-200  flex flex-col justify-between">
        ...
        <div>
          ...
          <button onClick={() => handleIsStacked()} className={`btn w-full btn-neutral ${isAlreadyAdded ? "cursor-not-allowed" : ""}`} >
